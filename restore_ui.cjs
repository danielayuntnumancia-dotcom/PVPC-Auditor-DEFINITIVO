...
const startStr = `                  {isValid && results ? (
                    <>
                  {/* Sello de Alerta Presupuestaria */}`;
const startReplacement = `                  {/* Sello de Alerta Presupuestaria */}`;
app = app.replace(startStr, startReplacement);
...
if (app.includes(endStr)) {
    app = app.replace(endStr, "");
} else {
    console.log("Could not find exact end string, trying regex.");
    const endRegex = /<\/s*>\s*\)\s*:\s*\(\s*<div[^>]*>\s*<div[^>]*>\s*<span[^>]*>Desglose Analítico<\/span>\s*<h3[^>]*>Réplica de Factura<\/h3>\s*<\/div>\s*<div[^>]*>\s*<AlertTriangle[^>]*\/>\s*<p[^>]*>Datos pendientes de corregir<\/p>\s*<\/div>\s*<\/div>\s*\)}/s;
    app = app.replace(endRegex, "");
}
